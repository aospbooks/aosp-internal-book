...
**ART: pantherlake x86 target.** ART adds Intel **Pantherlake** as a known x86 / x86-64 instruction-set variant (`art/runtime/arch/x86/instruction_set_features_x86.cc` lines 43-115), registered with SSSE3, SSE4.1/4.2, POPCNT, and AVX/AVX2 (the same tier as `alderlake`/`kabylake`), so `dex2oat` can emit AVX2 vectorized code for these CPUs.
...
**`system/memory/mmd` (Modern Memory Daemon).** Android 17 introduces `mmd`, a new native Rust daemon that centralizes Android memory-management configuration and ZRAM tunables (`system/memory/mmd/README.md`). Previously ZRAM setup was fragmented across `swapon_all`, `config.xml` overlays and `ro.zram.*` properties; `mmd` unifies this and separates swap management from system_server. It registers a Binder service named `mmd` (`system/memory/mmd/src/main.rs:203-205`) exposing the `IMmd` AIDL interface (`system/memory/mmd/aidl/android/os/IMmd.aidl`). A second, dedicated `mmd_setup` service (`system/memory/mmd/src/mmd_setup.rs`), started by init, performs first-boot ZRAM activation. When `mmd.zram.enabled` is set, ZRAM setup inside `swapon_all` becomes a no-op and the legacy overlay config is ignored.
...
The genuinely new low-memory path is **per-process** ZRAM operations. `IMmd` gained `supportsProcessMemoryZramOps()`, `asyncWritebackProcessZramMemory(pidfd, callback)` and `asyncPrefetchProcessZramMemory(pidfd)` (`system/memory/mmd/aidl/android/os/IMmd.aidl:51-80`). Writeback targets a single process by `pidfd`, pushes its ZRAM-resident pages to the writeback backing device, and reports a `WritebackStatus` plus bytes written through `IMmdProcessWritebackCallback`. Prefetch is the inverse: it pulls a process's written-back pages back into the compressed pool, useful before a cached app is resumed. These ride new zram kernel ioctls (`ZRAM_ANDROID_IOC_PROCESS_WRITEBACK_CMD`, `..._PREFETCH_CMD`) wrapped in `system/memory/mmd/src/zram/per_process_ioctls.rs`.
...
**`hardware/nxp/uwb` -- NXP UWB vendor HAL.** A newly added repository providing a concrete vendor implementation of the `android.hardware.uwb` AIDL HAL for NXP's SR1XX UWB chipset family. Previously the AOSP tree shipped only the HAL interface and a stub; this repo carries a shippable implementation OEMs using NXP silicon can build directly.

The AIDL service implements `IUwb`/`IUwbChip` (`hardware/nxp/uwb/aidl/uwb.h:33`, `uwb_chip.h:35`) and registers as `android.hardware.uwb-service.nxp` running as the `vendor.uwb_hal` init service under the `uwb` user (`hardware/nxp/uwb/aidl/nxp-uwb-service.rc`, `nxp-uwb-service.xml` declaring `android.hardware.uwb`/`IUwb` v1). The thin AIDL layer bridges into the legacy NXP HAL core: `UwbChip::open()` calls `phNxpUciHal_open`, `coreInit()` calls `phNxpUciHal_coreInitialization`, and `sendUciMessage()` forwards UCI (UWB Command Interface) frames via `phNxpUciHal_write` (`hardware/nxp/uwb/aidl/uwb_chip.cpp:69,90,101`). The bulk of the logic lives under `halimpl/` -- HBCI firmware download, TML transport, calibration (`phNxpUwbCalib.cc`), session tracking (`sessionTrack.cc`), time-sync (`timeSync.cc`) -- with board profiles such as `example_config/SR1XX`. This repo is what makes UWB hardware ranging work on NXP-based devices feeding the ranging stack below.
...
**`external/lfi/*` — LFI toolchain (external, integration only).** Upstream components vendored to back the in-process sandbox: `lfi-verifier` (`liblfiv.a` + `lfi-verify`, validates that an ELF only emits sandbox-safe instructions, with `LFI_BOX_FULL` vs `LFI_BOX_STORES` box types), `lfi-runtime` (`liblfi`: reserves and maps the sandbox address space and transfers control in/out, plus Linux host-call emulation), `lfi-bind` (Go tool generating sandbox init code and trampolines), `rlbox` + `rlbox-lfi` (the RLBox API with an LFI backend), and `disarm`/`fadec` (zero-dependency AArch64/x86 decoders used by the verifier). Integrated, not modified — the build links `liblfi` and the sandboxed `libopus_lfi` into `frameworks/av/media/module/libapexcodecs`, and codec2 selects the in-process path when `android.media.codec.in_process_sw_codec_lfi` is set (`frameworks/av/media/codec2/hal/client/client.cpp:1909,2034`; flag in `frameworks/av/media/aconfig/codec_fwk.aconfig`). The capability: run an untrusted software decoder inside the app/codec process with memory safety enforced by verified code instead of a separate sandbox process.

**`external/keyattestation` — Android Key Attestation verifier (external, integration only).** A Kotlin library (`Android.bp` builds `java_library "keyattestation"`, visible to `//frameworks/base/core/java` and vendor) verifying key-attestation certificate chains produced by KeyMint/RKP. Its `Verifier` takes trust-anchor, revoked-serial, and time sources and returns a `VerificationResult` exposing the attested public key, security level, verified-boot state, and device info, with pluggable `ChallengeChecker`s. It ships its own root set (`roots.json`, `keyattestation_roots` filegroup). Integrated as the in-tree verification path for attestation chains; mirrored from `github.com/android/keyattestation`.
...
**Desktop windowing in Launcher.** `TaskbarDesktopModeController` (`quickstep/src/com/android/launcher3/taskbar/TaskbarDesktopModeController.kt`) listens to `DesktopVisibilityController` and exposes display-scoped queries (`isInDesktopMode(displayId)`, active-desk tracking). `TaskbarRecentAppsController.kt` changes the taskbar's app row by mode: fullscreen shows the N most-recent tasks, desktop mode shows the currently *running* tasks as `GroupTask`s. Desktop app launches route through dedicated remote transitions in the new `com.android.launcher3.desktop` package (`DesktopAppLaunchTransition.kt`, `DesktopAppLaunchTransitionManager.kt`, `DesktopAppLaunchAnimatorHelper.kt`, `DesktopRecentsTransitionController.kt`), including a minimize animation when a window is moved to back during launch. Desktop mode is resolved through `DesktopStateProvider` (`.../launcher3/desktop/DesktopStateProvider.kt`), with gating now flowing through the platform `android.window.DesktopExperienceFlags` mechanism rather than ad-hoc Launcher flags.
...
`external/openxr-sdk` (Khronos OpenXR SDK, `release-1.1.50`, upgraded 2025-09-23 per `METADATA`) is newly vendored, integrated minimally and read-only: the in-tree `Android.bp` exposes a single `cc_library_headers { name: "openxr_headers" }` over `include/`, i.e. the OpenXR headers (not the loader) are made available to native consumers. This is the platform-side signal of Android XR runtime support and lines up with the new framework feature strings in `frameworks/base/core/api/current.txt`: `FEATURE_XR_API_OPENXR` ("android.software.xr.api.openxr") and `FEATURE_XR_API_SPATIAL`, both gated by the `android.xr.xr_manifest_entries` flag. The SDK provides the standard XR ABI; vendor runtimes implement it, and OpenXR apps declare the feature to require an XR device.
...
**Soong API / compliance database (`soong_api`).** The biggest build-system change is a new parallel singleton `soong_api_db` (`build/soong/soong_api/soong_api.go`) that walks every module proxy and emits a per-module `SoongApiModuleRecord`: identity and type (including resolved `base_type`), install/built files, license and `license_kind` metadata, `trendy_team_id`, test-only/top-level flags, language-specific dependency edges for Java/CC/Rust (libs, static/whole-static/header libs, CRT), and CIPD source. `exportRecords` writes three artifacts to `out/soong/soong_api/<product>/`: `soong_api.json`, `soong_api.zip`, and a SQLite `soong_api.db`. This is the successor to the older `metadata.db`/`metadata_db_loader` (both renamed to the `soong_api` family), and underpins SBOM and provenance generation rather than the legacy native-gRPC query path that was removed.
...
Android 17's marquee device-support change is the **Software Defined Vehicle (SDV)** platform. Rather than a single repo, SDV arrives as an entire new top-level tree (`system/software_defined_vehicle/`) plus a flagship reference device (`device/google/sdv`), a new HAL interface package (`hardware/sdv/interfaces`), and an automotive display-safety service (`packages/services/display_safety`). The model is a *headless* vehicle Android OS: SDV "Core" runs vehicle services in their own VM with no UI, communicating with one or more AAOS In-Vehicle Infotainment (IVI) VMs and non-Android automotive ECUs over SOME/IP.
...
- `guest/commands/vkms_controller/` is a new guest-side binary for Virtual Kernel Mode Setting (`main.cpp`). It consolidates previously fragmented VKMS logic so the host CLI is a stateless `adb shell vkms_controller ...` proxy, test frameworks share one setup path, and the guest correlates virtual hardware (ConfigFS indices) to SurfaceFlinger display IDs. Ships as a `vendor` `cc_binary` (`guest/commands/vkms_controller/Android.bp`).
